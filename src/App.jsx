...
const GLOBAL_CSS = `
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  :root{
    ...
    --purple:#6e6efc;
    --purple-hover:#5a5af0;
    --purple-light:rgba(110,110,252,.09);
    --purple-border:rgba(110,110,252,.28);
    --red:#dc2626;
    --teal:#0d9488;
    --amber:#f59e0b;
    --indigo:#6e6efc;
  ...
  /* ── Base elements ── */
  textarea{background:var(--bg);border:1.5px solid var(--border);border-radius:10px;color:var(--text);font-family:'Plus Jakarta Sans',sans-serif;font-size:13px;line-height:1.7;padding:13px;resize:none;width:100%;outline:none;transition:border-color .2s,box-shadow .2s}
  textarea:focus{border-color:var(--purple);box-shadow:0 0 0 3px rgba(110,110,252,.1)}
  input,select{background:var(--bg);border:1.5px solid var(--border);border-radius:10px;color:var(--text);font-family:'Plus Jakarta Sans',sans-serif;font-size:13px;padding:10px 14px;outline:none;transition:border-color .2s,box-shadow .2s;width:100%}
  input:focus,select:focus{border-color:var(--purple);box-shadow:0 0 0 3px rgba(110,110,252,.1)}
  input[type=checkbox]{width:auto;cursor:pointer} input[type=date]{color-scheme:light}
  select option{background:var(--bg)}

  /* ── Buttons ── */
  .btn{border:none;cursor:pointer;font-family:'Plus Jakarta Sans',sans-serif;font-size:12px;font-weight:600;letter-spacing:.2px;padding:9px 18px;border-radius:8px;transition:all .2s}
  .gold{background:var(--purple);color:#fff;font-weight:600;box-shadow:0 2px 10px rgba(110,110,252,.25)}.gold:hover{background:var(--purple-hover);transform:translateY(-1px);box-shadow:0 4px 18px rgba(110,110,252,.35)}
  .ghost{background:transparent;color:var(--text2);border:1.5px solid var(--border)}.ghost:hover{border-color:var(--border2);color:var(--text);background:var(--bg2)}
  .grn{background:rgba(13,148,136,.08);color:var(--teal);border:1.5px solid rgba(13,148,136,.22)}.grn:hover{background:rgba(13,148,136,.14)}
  .red-btn{background:rgba(220,38,38,.07);color:var(--red);border:1.5px solid rgba(220,38,38,.2)}.red-btn:hover{background:rgba(220,38,38,.12)}

  /* ── Nav tabs ── */
  .tab{background:none;border:none;cursor:pointer;padding:7px 14px;font-family:'Plus Jakarta Sans',sans-serif;font-size:10px;font-weight:600;letter-spacing:.4px;text-transform:uppercase;transition:all .2s;border-radius:8px;color:var(--muted)}
  .tab:hover{color:var(--text2);background:var(--bg3)}
  .tab.on{background:var(--purple)!important;color:#fff!important;border:none}

  /* ── Review card ── */
  .rc{background:var(--bg);border:1.5px solid var(--border);border-radius:12px;padding:14px;cursor:pointer;transition:all .2s}
  .rc:hover{border-color:var(--border2);box-shadow:0 2px 12px rgba(17,17,24,.06)}
  .rc.sel{border-color:var(--purple)!important;background:rgba(110,110,252,.04)!important;box-shadow:0 0 0 3px rgba(110,110,252,.08)!important}

  /* ── Sidebar nav ── */
  .nav{background:none;border:none;cursor:pointer;padding:10px 14px;font-family:'Plus Jakarta Sans',sans-serif;font-size:13px;font-weight:500;color:var(--muted);transition:all .2s;width:100%;text-align:left;border-radius:10px;display:flex;align-items:center;gap:10px}
  .nav:hover{background:var(--bg2);color:var(--text2)}
  .nav.on{background:var(--purple)!important;color:#fff!important;font-weight:600}

  /* ── Toggle ── */
  .tog{position:relative;display:inline-block;width:44px;height:24px;flex-shrink:0}
  .tog input{opacity:0;width:0;height:0}
  .sli{position:absolute;cursor:pointer;top:0;left:0;right:0;bottom:0;background:var(--bg3);border-radius:24px;transition:.3s;border:1.5px solid var(--border2)}
  ...
  input:checked+.sli{background:rgba(110,110,252,.15);border-color:rgba(110,110,252,.35)}
  input:checked+.sli:before{transform:translateX(20px);background:var(--purple)}
  ...
  /* ── Period filter ── */
  .period-btn{background:var(--bg);border:1.5px solid var(--border);border-radius:8px;padding:7px 13px;font-family:'Plus Jakarta Sans',sans-serif;font-size:12px;font-weight:500;color:var(--muted);cursor:pointer;display:flex;align-items:center;gap:8px;transition:all .2s;white-space:nowrap}
  .period-btn:hover{border-color:var(--border2);color:var(--text2)}
  .period-btn.active-period{border-color:var(--purple-border);color:var(--purple);background:var(--purple-light)}
  .period-dropdown{position:absolute;top:calc(100% + 8px);right:0;background:var(--bg);border:1.5px solid var(--border);border-radius:12px;overflow:hidden;z-index:100;min-width:200px;box-shadow:0 8px 32px rgba(17,17,24,.1)}
  .period-opt{display:block;width:100%;background:none;border:none;text-align:left;padding:10px 16px;font-family:'Plus Jakarta Sans',sans-serif;font-size:12px;font-weight:500;color:var(--muted);cursor:pointer;transition:all .15s}
  .period-opt:hover{background:var(--purple-light);color:var(--purple)}
  .period-opt.sel-opt{color:var(--purple);background:var(--purple-light);font-weight:600}
  .custom-date-row{display:flex;gap:8px;padding:10px 14px;border-top:1.5px solid var(--border);background:var(--bg2)}

  /* ── Auth inputs ── */
  .auth-input{background:var(--bg)!important;border:1.5px solid var(--border)!important;border-radius:10px!important;color:var(--text)!important;font-family:'Plus Jakarta Sans',sans-serif!important;font-size:13px!important;padding:13px 16px!important;outline:none!important;transition:border-color .2s,box-shadow .2s!important;width:100%!important}
  .auth-input:focus{border-color:var(--purple)!important;box-shadow:0 0 0 3px rgba(110,110,252,.1)!important}
  .auth-input.err{border-color:rgba(220,38,38,.5)!important}
  .auth-btn{width:100%;padding:14px;background:var(--purple);border:none;border-radius:10px;color:#fff;font-family:'Plus Jakarta Sans',sans-serif;font-size:13px;font-weight:600;cursor:pointer;transition:all .2s;margin-top:4px;box-shadow:0 3px 14px rgba(110,110,252,.28)}
  .auth-btn:hover{background:var(--purple-hover);transform:translateY(-1px);box-shadow:0 5px 20px rgba(110,110,252,.35)}
  .auth-btn:active{transform:translateY(0)} .auth-btn:disabled{opacity:.45;cursor:not-allowed;transform:none;box-shadow:none}
  .auth-link{background:none;border:none;cursor:pointer;font-family:'Plus Jakarta Sans',sans-serif;font-size:12px;color:var(--purple);text-decoration:underline;text-decoration-color:rgba(110,110,252,.3);transition:color .2s}
  .auth-link:hover{color:var(--purple-hover)}
  .field-err{font-family:'Plus Jakarta Sans',sans-serif;font-size:11px;color:var(--red);margin-top:5px}

  /* ── Toasts ── */
  .success-toast{position:fixed;top:24px;right:24px;background:var(--teal);color:#fff;font-family:'Plus Jakarta Sans',sans-serif;font-size:12px;font-weight:600;padding:10px 18px;border-radius:8px;z-index:9999;animation:fi .3s ease;box-shadow:0 4px 16px rgba(13,148,136,.3)}
  .error-toast{position:fixed;top:24px;right:24px;background:var(--red);color:#fff;font-family:'Plus Jakarta Sans',sans-serif;font-size:12px;font-weight:600;padding:10px 18px;border-radius:8px;z-index:9999;animation:fi .3s ease;box-shadow:0 4px 16px rgba(220,38,38,.3)}
  ...
  /* ── Google button ── */
  .goog-btn{display:flex;align-items:center;gap:12px;width:100%;padding:14px 18px;background:#fff;border:1.5px solid #dadce0;border-radius:10px;cursor:pointer;font-family:'Plus Jakarta Sans',sans-serif;font-size:13px;font-weight:500;color:#3c4043;transition:all .2s;justify-content:center}
  .goog-btn:hover{background:#f8f9fa;box-shadow:0 2px 16px rgba(0,0,0,.1);border-color:#c6c9cc}

  /* ── Perm / delete ── */
  .perm-row{display:flex;align-items:flex-start;gap:12px;padding:12px 0;border-bottom:1.5px solid var(--border)}
  .perm-row:last-child{border-bottom:none}
  .del-input{background:var(--bg)!important;border:1.5px solid rgba(220,38,38,.3)!important;border-radius:10px!important;color:var(--text)!important;font-family:'Plus Jakarta Sans',sans-serif!important;font-size:13px!important;padding:12px 14px!important;outline:none!important;width:100%!important;transition:border-color .2s!important}
  ...
  .loading-dot{width:8px;height:8px;border-radius:50%;background:var(--purple);animation:pulse 1.4s ease-in-out infinite}
...
const Stars = ({rating, size=14}) => (
  <span style={{display:"inline-flex",gap:1}}>
    {[1,2,3,4,5].map(s=>(
      <svg key={s} width={size} height={size} viewBox="0 0 24 24" fill={s<=rating?"var(--amber)":"var(--border2)"}>
        <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/>
      </svg>
    ))}
  </span>
);
const SC = {positive:"var(--teal)",neutral:"var(--amber)",negative:"var(--red)"};
const SB = {positive:"rgba(13,148,136,.1)",neutral:"rgba(245,158,11,.1)",negative:"rgba(220,38,38,.08)"};
...
const Card = ({children, style}) => (
  <div style={{background:"var(--bg)",border:"1.5px solid var(--border)",borderRadius:10,padding:"22px 26px",marginBottom:18,...style}}>{children}</div>
);

const AuthBG = () => (
  <div style={{position:"absolute",inset:0,overflow:"hidden",pointerEvents:"none"}}>
    <div style={{position:"absolute",top:"-10%",right:"-5%",width:"55vw",height:"55vw",borderRadius:"50%",background:"radial-gradient(circle,rgba(110,110,252,.06) 0%,transparent 65%)",filter:"blur(50px)"}}/>
    <div style={{position:"absolute",bottom:"-10%",left:"-5%",width:"40vw",height:"40vw",borderRadius:"50%",background:"radial-gradient(circle,rgba(13,148,136,.05) 0%,transparent 65%)",filter:"blur(50px)"}}/>
    <svg style={{position:"absolute",inset:0,width:"100%",height:"100%",opacity:.05}} xmlns="http://www.w3.org/2000/svg">
      <defs><pattern id="dots" width="24" height="24" patternUnits="userSpaceOnUse"><circle cx="1" cy="1" r="1" fill="rgba(110,110,252,.6)"/></pattern></defs>
  ...
  <div style={{textAlign:"center",marginBottom:32}}>
    <div style={{display:"inline-flex",flexDirection:"column",alignItems:"center",gap:2}}>
      <div style={{display:"flex",gap:6,alignItems:"baseline"}}>
        <span className="purple-text" style={{fontFamily:"'Syne',sans-serif",fontSize:32,fontWeight:700,letterSpacing:2}}>Review</span>
        <span style={{fontFamily:"'Syne',sans-serif",fontSize:32,fontWeight:700,color:"var(--text)",letterSpacing:2}}>Copilot</span>
      </div>
      <div style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontSize:8,color:"var(--muted)",letterSpacing:3,textTransform:"uppercase"}}>AI-Powered · Google Reviews</div>
    </div>
  </div>
);
...
function Modal({ onClose, children, danger }) {
  return (
    <div className="overlay-in" style={{position:"fixed",inset:0,background:"rgba(0,0,0,.75)",backdropFilter:"blur(4px)",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center",padding:24}}
      ...
      <div className="modal-in" style={{background:"var(--bg2)",border:`1px solid ${danger?"rgba(220,38,38,.14)":"var(--border2)"}`,borderRadius:14,padding:"32px",maxWidth:440,width:"100%",position:"relative"}}>
...
const PW_COLORS = ["var(--red)","var(--red)","var(--amber)","var(--teal)","var(--teal)"];
  ...
  return (
    <div style={{minHeight:"100vh",background:"var(--bg)",display:"flex",alignItems:"center",justifyContent:"center",position:"relative",fontFamily:"'Plus Jakarta Sans',sans-serif"}}>
      ...
      <div className="auth-in" style={{width:"100%",maxWidth:420,padding:"0 24px",position:"relative",zIndex:1}}>
        <AuthLogo/>
        <div style={{background:"var(--bg)",border:"1.5px solid var(--border)",borderRadius:18,padding:"36px 36px 32px"}}>
          <div style={{marginBottom:28}}>
            <div style={{fontFamily:"'Syne',sans-serif",fontSize:24,fontWeight:300,color:"var(--text)",marginBottom:6}}>Welcome back</div>
            <div style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontSize:10,color:"var(--muted)",letterSpacing:.5}}>Sign in to your account to continue</div>
          </div>
          {err&&<div className="fi" style={{marginBottom:16,padding:"10px 14px",background:"rgba(220,38,38,.06)",border:"1px solid #ef444430",borderRadius:7,display:"flex",alignItems:"center",gap:8}}><span style={{color:"var(--red)",fontSize:12}}>⚠</span><span style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontSize:10,color:"var(--red)"}}>{err}</span></div>}
          ...
          <div style={{textAlign:"center",marginTop:22,fontFamily:"'Plus Jakarta Sans',sans-serif",fontSize:10,color:"var(--muted)"}}>
      ...
      <div className="auth-in" style={{width:"100%",maxWidth:460,padding:"0 24px",position:"relative",zIndex:1}}>
        ...
        <div style={{background:"var(--bg)",border:"1.5px solid var(--border)",borderRadius:18,padding:"36px 36px 32px"}}>
          <div style={{marginBottom:28}}>
            ...
            <div style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontSize:10,color:"var(--muted)",letterSpacing:.5}}>Start managing your Google reviews with AI</div>
          </div>
          <div style={{display:"flex",flexDirection:"column",gap:14}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
              ...
              <div>
                <Label>Business Type <span style={{color:"var(--muted)"}}>(optional)</span></Label>
            ...
            <div style={{display:"flex",alignItems:"flex-start",gap:10,padding:"12px 14px",background:errs.agree?"rgba(220,38,38,.04)":"var(--bg)",border:`1px solid ${errs.agree?"rgba(220,38,38,.14)":"var(--border)"}`,borderRadius:8,cursor:"pointer"}} onClick={()=>{setAgree(a=>!a);setErrs(p=>({...p,agree:""}));}}>
              <div style={{width:16,height:16,borderRadius:4,border:`1px solid ${agree?"var(--purple)":"var(--border2)"}`,background:agree?"var(--purple)":"transparent",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,marginTop:1,transition:"all .2s"}}>
                {agree&&<svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="var(--bg)" strokeWidth="3"><polyline points="20 6 9 17 4 12"/></svg>}
              </div>
              <div style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontSize:9,color:"var(--muted)",lineHeight:1.7}}>I agree to the <span style={{color:"var(--purple)"}}>Terms of Service</span> and <span style={{color:"var(--purple)"}}>Privacy Policy</span></div>
            </div>
          ...
          {!sent ? (
            <>
              <div style={{marginBottom:28}}>
                ...
                <div style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontSize:10,color:"var(--muted)",letterSpacing:.5}}>We'll send a reset link to your email</div>
              </div>
              {err&&<div className="fi" style={{marginBottom:16,padding:"10px 14px",background:"rgba(220,38,38,.06)",border:"1px solid #ef444430",borderRadius:7}}><span style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontSize:10,color:"var(--red)"}}>{err}</span></div>}
              <Label>Email Address</Label>
              <input className="auth-input" type="email" placeholder="you@business.com" value={email} onChange={e=>{setEmail(e.target.value);setErr("");}} onKeyDown={e=>e.key==="Enter"&&submit()}/>
            ...
            <div className="fi" style={{textAlign:"center",padding:"10px 0"}}>
              ...
              <div style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontSize:10,color:"var(--muted)",lineHeight:1.8}}>A reset link has been sent to<br/><span style={{color:"var(--purple)"}}>{email}</span></div>
            </div>
          )}
          <div style={{textAlign:"center",marginTop:24,fontFamily:"'Plus Jakarta Sans',sans-serif",fontSize:10,color:"var(--muted)"}}>
...
const GOOGLE_PERMISSIONS = [
  {icon:"⭐",color:"var(--amber)",title:"Read your Google reviews",desc:"Access all reviews posted on your Google Business Profile in real time."},
  {icon:"✏",color:"var(--purple)",title:"Post replies on your behalf",desc:"Allow the copilot to publish AI-generated replies directly to Google."},
  {icon:"🏢",color:"var(--teal)",title:"View business profile info",desc:"Read your business name, address, category and operating hours."},
  {icon:"📊",color:"var(--purple)",title:"Access review analytics",desc:"Retrieve rating history and review trends to power insights."},
];
          ...
          <div style={{background:"var(--bg)",border:"1.5px solid var(--border)",borderRadius:18,padding:"36px 36px 32px"}}>
            <div style={{display:"flex",alignItems:"center",gap:14,marginBottom:28}}>
              ...
              <div>
                ...
                <div style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontSize:9,color:"var(--muted)",marginTop:3}}>One-time setup · takes 30 seconds</div>
              </div>
            </div>
            ...
            <div style={{background:"var(--bg)",border:"1.5px solid var(--border)",borderRadius:10,padding:"6px 16px",marginBottom:24}}>
            ...
            <div style={{padding:"12px 16px",background:"rgba(110,110,252,.04)",border:"1.5px solid rgba(110,110,252,.15)",borderRadius:8,marginBottom:24,fontFamily:"'Plus Jakarta Sans',sans-serif",fontSize:9,color:"rgba(110,110,252,.5)",lineHeight:1.7}}>
            ...
            <div style={{textAlign:"center",marginTop:20}}>
              <button className="auth-link" onClick={onSkip} style={{fontSize:9,color:"var(--muted)"}}>Skip for now — connect later in Settings</button>
            </div>
          </div>
        )}
        {step==="authorizing"&&(
          <div className="fi" style={{background:"var(--bg)",border:"1.5px solid var(--border)",borderRadius:18,padding:"48px 36px",textAlign:"center"}}>
            ...
            <div style={{height:4,background:"var(--border)",borderRadius:2,overflow:"hidden",marginBottom:12}}>
              <div style={{height:"100%",width:`${progress}%`,background:"linear-gradient(90deg,#4285f4,var(--purple))",borderRadius:2,transition:"width .2s ease"}}/>
            </div>
            <div style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontSize:9,color:"var(--muted)"}}>{Math.round(progress)}% complete</div>
          </div>
        )}
        {step==="success"&&(
          <div className="fi" style={{background:"var(--bg2)",border:"1.5px solid rgba(13,148,136,.25)",borderRadius:18,padding:"48px 36px",textAlign:"center"}}>
            <div style={{width:64,height:64,borderRadius:"50%",background:"rgba(13,148,136,.13)",border:"2px solid #10b98140",display:"flex",alignItems:"center",justifyContent:"center",fontSize:28,margin:"0 auto 20px"}}>✓</div>
  ...
  return (
    <div style={{fontFamily:"'Plus Jakarta Sans',sans-serif",background:"var(--bg)",minHeight:"100vh",color:"var(--text)",position:"relative"}}>
      ...
      {showDisconnectModal&&(
        <Modal onClose={()=>setShowDisconnectModal(false)} danger>
          <div style={{textAlign:"center",marginBottom:24}}>
            <div style={{width:52,height:52,borderRadius:"50%",background:"rgba(220,38,38,.07)",border:"1px solid #ef444430",display:"flex",alignItems:"center",justifyContent:"center",fontSize:22,margin:"0 auto 16px"}}>⚠</div>
        ...
        <Modal onClose={()=>{setShowDeleteModal(false);setDeleteConfirmText("");}} danger>
          ...
          <div style={{display:"flex",gap:10}}>
            ...
            <button className="btn" disabled={deleteConfirmText!==session.user.email||deleteLoading}
              style={{flex:1,background:deleteConfirmText===session.user.email?"var(--red)":"rgba(220,38,38,.09)",color:deleteConfirmText===session.user.email?"#fff":"rgba(220,38,38,.3)",border:"1px solid #ef444430",cursor:deleteConfirmText!==session.user.email?"not-allowed":"pointer",transition:"all .2s"}}
      ...
      <div style={{display:"flex",height:"100vh"}}>
        {/* Sidebar */}
        <div style={{width:215,background:"var(--bg)",borderRight:"1.5px solid var(--border)",display:"flex",flexDirection:"column",padding:"22px 10px",flexShrink:0}}>
          <div style={{padding:"0 8px 22px",borderBottom:"1.5px solid var(--border)"}}>
            <div className="purple-text" style={{fontFamily:"'Syne',sans-serif",fontSize:22,fontWeight:700,letterSpacing:1}}>Review</div>
            <div style={{fontFamily:"'Syne',sans-serif",fontSize:22,fontWeight:700,color:"var(--text)",letterSpacing:1}}>Copilot</div>
            <div style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontSize:8,color:"var(--muted)",letterSpacing:2,marginTop:3}}>AI-POWERED</div>
          </div>
          ...
          <div style={{padding:"14px 8px 0",borderTop:"1.5px solid var(--border)"}}>
            {profile?.google_connected?(
              <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:12,padding:"8px 10px",background:"rgba(13,148,136,.06)",border:"1px solid #10b98120",borderRadius:7}}>
              ...
              <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:12,padding:"8px 10px",background:"rgba(245,158,11,.04)",border:"1px solid #f59e0b20",borderRadius:7,cursor:"pointer"}} onClick={()=>setShowReconnectScreen(true)}>
                <div style={{width:6,height:6,borderRadius:"50%",background:"var(--amber)",flexShrink:0}}/>
                <div style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontSize:8,color:"var(--amber)",lineHeight:1.4}}>Connect Google →</div>
              </div>
            )}
            <div style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontSize:9,color:"var(--muted)",paddingLeft:6,marginBottom:4}}>{profile?.name||session.user.email}</div>
            <div style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontSize:8,color:"var(--muted)",paddingLeft:6,marginBottom:10}}>{biz||"No business set"}</div>
          </div>
        </div>
        ...
        <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
          {/* Header */}
          <div style={{padding:"16px 28px",borderBottom:"1.5px solid var(--border)",display:"flex",alignItems:"center",justifyContent:"space-between",background:"var(--bg2)",flexShrink:0}}>
            <div>
              ...
              <div style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontSize:9,color:"var(--muted)",letterSpacing:1,marginTop:2}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:14}}>
              ...
              <div className="user-menu" style={{position:"relative"}}>
                <button onClick={()=>setShowUserMenu(s=>!s)} style={{width:34,height:34,borderRadius:"50%",background:"var(--purple)",border:"none",cursor:"pointer",fontFamily:"'Plus Jakarta Sans',sans-serif",fontSize:11,color:"var(--bg)",fontWeight:700,display:"flex",alignItems:"center",justifyContent:"center"}}>
                  {initials}
                </button>
                {showUserMenu&&(
                  <div className="fi" style={{position:"absolute",top:"calc(100% + 10px)",right:0,background:"var(--bg2)",border:"1px solid #2a2a3a",borderRadius:10,padding:"12px",minWidth:200,zIndex:50,boxShadow:"0 8px 32px rgba(0,0,0,.5)"}}>
                    <div style={{padding:"4px 8px 12px",borderBottom:"1.5px solid var(--border)",marginBottom:8}}>
                    ...
                    <div style={{borderTop:"1.5px solid var(--border)",marginTop:4,paddingTop:4}}>
          ...
          <div style={{padding:"24px 28px",flex:1,overflowY:"auto"}}>
            ...
            {view==="dashboard"&&(
              <div className="fi">
                {reviews.length===0&&!revLoading&&(
                  <div style={{background:"rgba(110,110,252,.05)",border:"1.5px solid rgba(110,110,252,.15)",borderRadius:12,padding:"28px 32px",marginBottom:24,display:"flex",alignItems:"center",gap:24}}>
                    <div style={{fontSize:36,flexShrink:0}}>✦</div>
                    <div>
                      <div style={{fontFamily:"'Syne',sans-serif",fontSize:22,fontWeight:300,color:"var(--purple)",marginBottom:6}}>Welcome to Review Copilot</div>
                      <div style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontSize:9,color:"var(--muted)",lineHeight:1.8}}>
                        ...
                        <button onClick={()=>setView("settings")} style={{background:"none",border:"none",cursor:"pointer",fontFamily:"'Plus Jakarta Sans',sans-serif",fontSize:9,color:"var(--purple)",textDecoration:"underline"}}>Settings</button>
                        {" "}to import live reviews, or{" "}
                        <button onClick={()=>setView("reviews")} style={{background:"none",border:"none",cursor:"pointer",fontFamily:"'Plus Jakarta Sans',sans-serif",fontSize:9,color:"var(--purple)",textDecoration:"underline"}}>add a test review</button>
                        {" "}to explore the features.
                      </div>
                    </div>
                  </div>
                )}
                {revLoading&&<div style={{textAlign:"center",padding:"60px 0",fontFamily:"'Plus Jakarta Sans',sans-serif",fontSize:9,color:"var(--muted)"}}>Loading reviews…</div>}
                {!revLoading&&(
                  <>
                    <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:14,marginBottom:24}}>
                      {[
                        {l:"Avg Rating",  v:stats.avg,      s:gPeriod.days===9999?"all time":gPeriod.label.toLowerCase(),a:"var(--purple)"},
                        {l:"Total Reviews",v:stats.total,   s:gPeriod.days===9999?"all time":gPeriod.label.toLowerCase(),a:"var(--purple)"},
                        ...
                        <div key={s.l} style={{background:"var(--bg)",border:"1.5px solid var(--border)",borderRadius:10,padding:"18px 20px"}}>
                          <Label>{s.l}</Label>
                    ...
                    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:18,marginBottom:22}}>
                      <Card style={{marginBottom:0}}>
                        ...
                        {periodReviews.length===0?<div style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontSize:13,color:"var(--muted)",fontStyle:"italic",textAlign:"center",padding:"20px 0"}}>No reviews in this period</div>
                      ...
                      <Card style={{marginBottom:0}}>
                        ...
                        {periodReviews.length===0?<div style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontSize:13,color:"var(--muted)",fontStyle:"italic",textAlign:"center",padding:"20px 0"}}>No reviews in this period</div>
                        :[{l:"Positive",c:"var(--teal)",v:stats.positive},{l:"Neutral",c:"var(--amber)",v:periodReviews.filter(r=>r.sentiment==="neutral").length},{l:"Negative",c:"var(--red)",v:stats.negative}].map(s=>(
                    ...
                    <Card>
                      ...
                      {periodReviews.length===0?<div style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontSize:13,color:"var(--muted)",fontStyle:"italic",textAlign:"center",padding:"10px 0"}}>No reviews in this period</div>
                      :periodReviews.slice(0,5).map(r=>(
                        <div key={r.id} style={{display:"flex",alignItems:"flex-start",gap:12,padding:"12px 0",borderBottom:"1.5px solid var(--border)",cursor:"pointer"}} onClick={()=>{setView("reviews");setSelected(r);}}>
                          ...
                          <div style={{flex:1,minWidth:0}}>
                            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}>
                              ...
                              <div style={{display:"flex",alignItems:"center",gap:8}}><Stars rating={r.rating} size={10}/><span style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontSize:8,color:"var(--muted)"}}>{new Date(r.created_at).toLocaleDateString()}</span></div>
                            </div>
            ...
            {view==="reviews"&&(
              <div className="fi" style={{display:"grid",gridTemplateColumns:"320px 1fr",gap:18,height:"calc(100vh - 110px)"}}>
                <div style={{display:"flex",flexDirection:"column",gap:0,overflow:"hidden",background:"var(--bg)",border:"1.5px solid var(--border)",borderRadius:10}}>
                  <div style={{padding:"12px 14px",borderBottom:"1.5px solid var(--border)",display:"flex",gap:5,flexWrap:"wrap"}}>
                  ...
                  <div style={{flex:1,overflow:"auto",display:"flex",flexDirection:"column",gap:7,padding:"10px 8px"}}>
                    {revLoading?<div style={{textAlign:"center",padding:"40px 0",fontFamily:"'Plus Jakarta Sans',sans-serif",fontSize:9,color:"var(--muted)"}}>Loading…</div>
                    :filtered.length===0?(
                      <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",flex:1,gap:10,padding:"40px 16px"}}>
                        ...
                        <div style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontSize:13,color:"var(--muted)",fontStyle:"italic",textAlign:"center"}}>
                        ...
                        <div style={{display:"flex",alignItems:"center",gap:9,marginBottom:8}}>
                          ...
                          <div style={{flex:1,minWidth:0}}>
                            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                              <span style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontSize:10,color:"#b0b4cc"}}>{r.author}</span>
                              <span style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontSize:8,color:"var(--muted)"}}>{new Date(r.created_at).toLocaleDateString()}</span>
                            </div>
                            <div style={{display:"flex",alignItems:"center",gap:5,marginTop:3}}>
                              <Stars rating={r.rating} size={10}/>
                              {r.flagged&&<span style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontSize:7,color:"var(--red)",background:"rgba(220,38,38,.07)",padding:"1px 5px",borderRadius:3}}>FLAGGED</span>}
                              {r.replied&&<span style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontSize:7,color:"var(--teal)",background:"rgba(13,148,136,.09)",padding:"1px 5px",borderRadius:3}}>REPLIED</span>}
                            </div>
                          </div>
                        </div>
                        <div style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontSize:12,color:"var(--muted)",lineHeight:1.5,display:"-webkit-box",WebkitLineClamp:2,WebkitBoxOrient:"vertical",overflow:"hidden"}}>{r.text}</div>
                ...
                <div style={{overflow:"auto"}}>
                  {!selected?(
                    <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",height:"100%",gap:10}}>
                      ...
                      <div style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontSize:15,color:"var(--muted)",fontStyle:"italic"}}>Select a review to respond</div>
                    </div>
                  ):(
                    <div className="fi">
                      <Card>
                        <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:16,marginBottom:14}}>
                          <div style={{display:"flex",alignItems:"center",gap:12}}>
                            ...
                            <div>
                              ...
                              <div style={{display:"flex",alignItems:"center",gap:8,marginTop:3}}><Stars rating={selected.rating} size={12}/><span style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontSize:8,color:"var(--muted)"}}>{new Date(selected.created_at).toLocaleDateString()}</span></div>
                            </div>
                          </div>
                          <button className="btn ghost" style={{fontSize:9,padding:"6px 12px",color:selected.flagged?"var(--red)":"var(--muted)",borderColor:selected.flagged?"rgba(220,38,38,.14)":"var(--border2)"}} onClick={()=>toggleFlag(selected.id)}>{selected.flagged?"⚑ Unflag":"⚑ Flag"}</button>
                        </div>
                        <div style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontSize:15,color:"#8890b0",lineHeight:1.7,padding:"14px 0",borderTop:"1.5px solid var(--border)",borderBottom:selected.replied?"1px solid #1e1e2e":"none"}}>"{selected.text}"</div>
                        {selected.replied&&selected.reply&&(
                          <div style={{marginTop:14,padding:"12px 15px",background:"rgba(13,148,136,.06)",borderLeft:"3px solid #10b981",borderRadius:"0 6px 6px 0"}}>
            ...
            {view==="insights"&&(
              <div className="fi" style={{maxWidth:740}}>
                ...
                {iLoading&&<div style={{textAlign:"center",padding:"60px 0",fontFamily:"'Syne',sans-serif",fontSize:20,color:"var(--muted)",fontStyle:"italic"}}>Analysing your reviews…</div>}
                {iData&&!iData.empty&&!iData.error&&!iLoading&&(
                  <div className="fi">
                    <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:12,marginBottom:18}}>
                      {[{l:"Reviews Analysed",v:iData.count,c:"var(--purple)"},{l:"Avg Rating",v:iData.avgRating+"★",c:"var(--purple)"},{l:"Urgency",v:(iData.urgency||"").toUpperCase(),c:iData.urgency==="high"?"var(--red)":iData.urgency==="medium"?"var(--amber)":"var(--teal)"}].map(s=>(
                        <div key={s.l} style={{background:"var(--bg)",border:"1.5px solid var(--border)",borderRadius:8,padding:"14px 18px"}}>
                          <Label>{s.l}</Label>
                    ...
                    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:14}}>
                      {[
                        {title:"Going Right",sub:"Strengths to maintain",icon:"✓",color:"var(--teal)",bg:"rgba(13,148,136,.04)",border:"rgba(13,148,136,.13)",arrow:"→",tc:"var(--muted)",items:iData.goingRight},
                        {title:"Going Wrong",sub:"Issues to address",icon:"✗",color:"var(--red)",bg:"rgba(220,38,38,.04)",border:"rgba(220,38,38,.09)",arrow:"→",tc:"var(--muted)",items:iData.goingWrong},
                        {title:"Improvements",sub:"Action items",icon:"↑",color:"var(--purple)",bg:"rgba(110,110,252,.04)",border:"rgba(110,110,252,.13)",arrow:"→",tc:"var(--muted)",items:iData.improvements},
                      ].map(col=>(
                ...
                {iData?.empty&&<div style={{textAlign:"center",padding:"60px 0",fontFamily:"'Syne',sans-serif",fontSize:20,color:"var(--muted)",fontStyle:"italic"}}>No reviews found for this period.</div>}
                ...
                {!iData&&!iLoading&&<div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"70px 0",gap:10}}><div style={{fontFamily:"'Syne',sans-serif",fontSize:50,color:"var(--border)"}}>◉</div><div style={{fontFamily:"'Syne',sans-serif",fontSize:21,color:"var(--muted)",fontStyle:"italic"}}>Select a time period and generate insights</div></div>}
              </div>
            )}
            ...
            {view==="settings"&&(
              <div className="fi" style={{maxWidth:660}}>
                <Card>
                  <div style={{display:"flex",alignItems:"center",gap:16,marginBottom:20,paddingBottom:20,borderBottom:"1.5px solid var(--border)"}}>
                    <div style={{width:52,height:52,borderRadius:"50%",background:"var(--purple)",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'Plus Jakarta Sans',sans-serif",fontSize:16,color:"var(--bg)",fontWeight:600,flexShrink:0}}>{initials}</div>
                    <div>
                      <div style={{fontFamily:"'Syne',sans-serif",fontSize:20,fontWeight:300}}>{profile?.name}</div>
                      <div style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontSize:9,color:"var(--muted)",marginTop:3}}>{session.user.email}</div>
                      <div style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontSize:8,color:"var(--muted)",marginTop:4}}>Member since {new Date(profile?.created_at||Date.now()).toLocaleDateString("en-US",{month:"long",year:"numeric"})}</div>
                    </div>
                    <button className="btn ghost" style={{marginLeft:"auto",fontSize:9,color:"var(--red)",borderColor:"rgba(220,38,38,.14)"}} onClick={onLogout}>Sign Out</button>
                  </div>
                ...
                <Card>
                  <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:18}}>
                    <div><div style={{fontFamily:"'Syne',sans-serif",fontSize:20,fontWeight:300}}>Email Notifications</div><div style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontSize:9,color:"var(--muted)",marginTop:3}}>Get alerted when reviews match your criteria</div></div>
                  ...
                  <div style={{opacity:nEnabled?1:.4,transition:"opacity .3s",pointerEvents:nEnabled?"auto":"none"}}>
                    ...
                    <div style={{display:"flex",flexDirection:"column",gap:9,marginTop:8}}>
                      {[{l:"Negative (1–2 stars)",sub:"Recommended — catch issues fast",v:nNeg,set:setNNeg,a:"var(--red)"},{l:"Neutral (3 stars)",sub:"Track mixed experiences",v:nNeu,set:setNNeu,a:"var(--amber)"},{l:"Any new review",sub:"Get notified for all reviews",v:nAll,set:setNAll,a:"var(--purple)"}].map(o=>(
                        <div key={o.l} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"11px 14px",background:"var(--bg)",border:`1px solid ${o.v?o.a+"30":"var(--border)"}`,borderRadius:8,transition:"border-color .2s"}}>
                          <div><div style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontSize:10,color:o.v?o.a:"var(--muted)"}}>{o.l}</div><div style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontSize:8,color:"var(--muted)",marginTop:2}}>{o.sub}</div></div>
                ...
                <Card>
                  <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:18}}>
                    <div><div style={{fontFamily:"'Syne',sans-serif",fontSize:20,fontWeight:300}}>Auto-Reply Copilot</div><div style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontSize:9,color:"var(--muted)",marginTop:3}}>Automatically generate &amp; post AI replies</div></div>
                  ...
                  <div style={{opacity:arEnabled?1:.4,transition:"opacity .3s",pointerEvents:arEnabled?"auto":"none"}}>
                    ...
                    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10,marginBottom:16,marginTop:8}}>
                      {[{l:"Positive",sub:"4–5 stars",v:arPos,set:setArPos,c:"var(--teal)",ic:"✓"},{l:"Neutral",sub:"3 stars",v:arNeu,set:setArNeu,c:"var(--amber)",ic:"~"},{l:"Negative",sub:"1–2 stars",v:arNeg,set:setArNeg,c:"var(--red)",ic:"!"}].map(o=>(
                        <div key={o.l} onClick={()=>o.set(!o.v)} style={{cursor:"pointer",padding:"14px",background:o.v?`${o.c}10`:"var(--bg)",border:`1px solid ${o.v?o.c+"38":"var(--border)"}`,borderRadius:9,transition:"all .2s",textAlign:"center"}}>
                          ...
                          <div style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontSize:8,color:"var(--muted)",marginTop:2}}>{o.sub}</div>
                        </div>
                      ))}
                    </div>
                    ...
                    <div style={{padding:"14px",background:"var(--bg)",border:"1.5px solid var(--border)",borderRadius:8}}>
                      ...
                      {arLog.length>0&&(<div className="fi" style={{marginTop:12,display:"flex",flexDirection:"column",gap:5}}>{arLog.map((log,i)=>(<div key={i} style={{padding:"9px 11px",background:log.type==="success"?"rgba(13,148,136,.06)":log.type==="error"?"rgba(220,38,38,.06)":"var(--border)",borderRadius:6,borderLeft:`3px solid ${log.type==="success"?"var(--teal)":log.type==="error"?"var(--red)":"var(--muted2)"}`}}><div style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontSize:9,color:log.type==="success"?"var(--teal)":log.type==="error"?"var(--red)":"var(--muted)"}}>{log.msg}</div>{log.preview&&<div style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontSize:11,color:"var(--muted)",marginTop:5,lineHeight:1.5,fontStyle:"italic"}}>"{log.preview.slice(0,130)}{log.preview.length>130?"…":""}"</div>}</div>))}</div>)}
                    </div>
                  </div>
                ...
                <Card style={{border:profile?.google_connected?"1.5px solid rgba(13,148,136,.25)":"1.5px solid rgba(110,110,252,.15)",background:profile?.google_connected?"rgba(13,148,136,.05)":"rgba(110,110,252,.03)"}}>
                  <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:16}}>
                    <div style={{display:"flex",alignItems:"center",gap:14}}>
                      <div style={{width:44,height:44,borderRadius:12,background:profile?.google_connected?"rgba(13,148,136,.09)":"linear-gradient(135deg,#4285f420,#34a85320)",border:`1px solid ${profile?.google_connected?"rgba(13,148,136,.2)":"#4285f430"}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:22,flexShrink:0}}>G</div>
                      <div>
                        <div style={{fontFamily:"'Syne',sans-serif",fontSize:18,fontWeight:300,color:profile?.google_connected?"var(--teal)":"var(--purple)"}}>Google Business Profile</div>
                        {profile?.google_connected?(
                          <div style={{display:"flex",alignItems:"center",gap:6,marginTop:4}}>
                            ...
                            <span style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontSize:9,color:"var(--muted)",margin:"0 4px"}}>·</span>
                    ...
                    {profile?.google_connected?(
                      <div style={{display:"flex",gap:8,flexShrink:0}}>
                        <button className="btn ghost" style={{fontSize:9,padding:"7px 13px",color:"var(--purple)",borderColor:"#7c8cf830"}} onClick={()=>setShowReconnectScreen(true)}>Reconnect</button>
                        <button className="btn ghost" style={{fontSize:9,padding:"7px 13px",color:"var(--red)",borderColor:"rgba(220,38,38,.14)"}} onClick={()=>setShowDisconnectModal(true)}>Disconnect</button>
                      </div>
                    ):(
                ...
                <div style={{background:"rgba(220,38,38,.04)",border:"1px solid #ef444420",borderRadius:10,padding:"22px 26px",marginTop:6}}>
  ...
  if(appState === "loading") {
    return (
      <div className="loading-screen" style={{fontFamily:"'Plus Jakarta Sans',sans-serif",position:"relative",overflow:"hidden"}}>
        <style>{GLOBAL_CSS}</style>
        <div style={{position:"absolute",top:"-20%",right:"-10%",width:"50vw",height:"50vw",borderRadius:"50%",background:"radial-gradient(circle,rgba(110,110,252,.07) 0%,transparent 70%)",filter:"blur(40px)",pointerEvents:"none"}}/>
        <div style={{position:"absolute",bottom:"-20%",left:"-10%",width:"40vw",height:"40vw",borderRadius:"50%",background:"radial-gradient(circle,rgba(13,148,136,.06) 0%,transparent 70%)",filter:"blur(40px)",pointerEvents:"none"}}/>
        <div style={{textAlign:"center",position:"relative",zIndex:1}}>
          <div className="purple-text" style={{fontFamily:"'Syne',sans-serif",fontSize:36,fontWeight:700,letterSpacing:2,marginBottom:8}}>Review Copilot</div>
